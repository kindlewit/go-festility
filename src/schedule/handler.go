...
func HandleCreateSchedule(c *gin.Context) {
	...
	for (constants.ScheduleIdHashLength != 0 && len(newScheduleId) == 0) || (IsUniqueScheduleId(newScheduleId)) {
	...
	success, err = CreateSchedule(newSchedule)
...
func HandleGetSchedule(c *gin.Context) {
	...
	var resp ScheduleApiRespWithSlots
	var slots []Slot
	...
	schedule, err = GetSchedule(festId, scheduleId)
	if err != nil {
		constants.HandleError(c, err)
		return
	}

	slots, err = GetScheduleSlots(scheduleId, limit, skip)
	if err != nil {
		constants.HandleError(c, err)
		return
	}

	resp = ScheduleApiRespWithSlots{
		Id:     schedule.Id,
		Fest:   schedule.Fest,
...
func HandleGetDailySchedule(c *gin.Context) {
	...
	scheduleId, err = GetDefaultScheduleId(festId)
	if err != nil {
		constants.HandleError(c, err)
		return
	}
	...
	resp, err = GetScheduleSlotsByTime(scheduleId, startOfDayAsUnix, endOfDayAsUnix)
	if err != nil {
		constants.HandleError(c, err)
		return
	}
